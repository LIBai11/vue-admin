...
//新建一个对象来存放添加的tag
// console.log(store.state.editArticleModule.tagNameList)
const tags: any[] = []
store.state.editArticleModule.tagNameList.forEach((tag: string) => {
    tags.push(tag)
})

const hasAddTags = ref()
hasAddTags.value = tags
//表单内容
const tagForm = ref({
    tagName: '',
})
