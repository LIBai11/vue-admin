<template>
    <div class="verify-comment">
        ...
        <div class="verify-content">
            <comment-content
                :table-value="commentsVerifyInfo"
...
import { CommentContent, MessageHeader, KxPagination } from '@/components'
...
const getComments = (payload: IMessageParams) => {
    ...
    params.value.isReview = 0
    store.dispatch('commentModule/getComments', payload)
}
const commentsVerifyInfo = computed<ICommentRecordList>(() => store.state.commentModule.recordList)
const verifyCounts = computed<number>(() => store.state.commentModule.count)
onMounted(() => {
    getComments({})
})
